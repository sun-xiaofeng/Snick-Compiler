/*
** Infrastructure for simulating Brill programs.
**
** Heavily based on the original emulator written by Zoltan Somogyi.
...
static  int     brilltrunc(float f);
static  int     brillround(float f);
...
main(int argc, char **argv)
{
    ...
    brillyyin = fopen(filename, "r");
    brillyyfile = filename;
    brillyylinenum = 1;
    if (brillyyin == NULL) {
        perror(filename);
        exit(EXIT_FAILURE);
...
usage(void) {
    ...
    printf("usage: brill [-i] [-q] [-s] filename\n");
    ...
    case OP_CALL_BUILTIN:
        ...
        switch (instr->func) {
            ...
            case FUNCOP_TRUNC:
                r1 = real_reg(0);
                init_all_regs();
                set_reg_int(instr->rd, brilltrunc(r1));
                break;

            case FUNCOP_ROUND:
                r1 = real_reg(0);
                init_all_regs();
                set_reg_int(instr->rd, brillround(r1));
                break;
