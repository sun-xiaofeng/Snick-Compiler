...
extern	char	*brillyytext;

extern	void	brillyyerror(const char *s);
%}
...
reg	:	TOKEN_REG
			...
			if ($1 < NUMBER_REGS) {
				...
				printf("%s, %d: ", brillyyfile, brillyylinenum);
...
void brillyyerror(const char *s)
{
	char		buf[80];
	...
	else if (brillyytext[0] == '\n' || brillyytext[0] == '\f')
		sprintf(buf, "%s at end of line", s);
	else if (isprint((int) brillyytext[0]))
		sprintf(buf, "%s at symbol `%s'", s, brillyytext);
	else
		sprintf(buf, "%s at \\%o", s, brillyytext[0]);
	
	printf("%s, %d: %s\n", brillyyfile, brillyylinenum, buf);
}
